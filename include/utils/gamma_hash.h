...
typedef struct gamma_hash_bucket
{
	uint32 hash;
...
#endif
{
	...
	tb->buckets = (gamma_hash_bucket *) SH_ALLOCATE(tb, sizeof(gamma_hash_bucket) * tb->size);
	tb->entries = (char *) SH_ALLOCATE(tb, sizeof(SH_ELEMENT_TYPE) * tb->size);
...
SH_GROW(SH_TYPE * tb, uint64 newsize)
{
	...
	gamma_hash_bucket *olddata = tb->buckets;
	gamma_hash_bucket *newdata;
	uint32		i;
	uint32		startelem = 0;
	uint32		copyelem;
	...
	tb->buckets =
		(gamma_hash_bucket *) SH_ALLOCATE(tb, sizeof(gamma_hash_bucket) * tb->size);
	...
	for (i = 0; i < oldsize; i++)
	{
		gamma_hash_bucket *oldentry = &olddata[i];
		uint32		hash;
		uint32		optimal;
		...
		hash = SH_ENTRY_HASH(tb, oldentry->entry);
	...
	for (i = 0; i < oldsize; i++)
	{
		gamma_hash_bucket *oldentry = &olddata[copyelem];
		...
		if (oldentry->entry != NULL)
		{
			...
			memcpy(newentry, oldentry, sizeof(gamma_hash_bucket));
		}
...
static inline void
gamma_hash_data_grow(SH_TYPE *tb)
{
	...
	tb->dataoffset = 0;
	tb->datasize = sizeof(SH_ELEMENT_TYPE) * (tb->size - tb->datacount);
	tb->datacount = tb->size;
...
restart:
	...
	while (true)
	{
		...
		uint32		curhash;
		uint32		curoptimal;
		gamma_hash_bucket *bucket = &data[curelem];
		SH_ELEMENT_TYPE *entry;
		...
		if (bucket->entry == NULL)
		{
			SH_ELEMENT_TYPE *newentry =
				(SH_ELEMENT_TYPE *) (tb->entries + tb->dataoffset);
			tb->dataoffset = tb->dataoffset + sizeof(SH_ELEMENT_TYPE);
			bucket->entry = newentry;

			tb->members++;
			newentry->SH_KEY = key;
#ifdef SH_STORE_HASH
			SH_GET_HASH(tb, newentry) = hash;
#endif
			newentry->status = SH_STATUS_IN_USE;
			*found = false;
			return newentry;
		}
		...
		{
			gamma_hash_bucket *lastbucket = bucket;
			...
			while (true)
			{
				gamma_hash_bucket *emptybucket;

				emptyelem = SH_NEXT(tb, emptyelem, startelem);
				emptybucket = &data[emptyelem];

				if (emptybucket->entry == NULL)
				{
			...
			{
				gamma_hash_bucket *movebucket;

				moveelem = SH_PREV(tb, moveelem, startelem);
				movebucket = &data[moveelem];

				memcpy(lastbucket, movebucket, sizeof(gamma_hash_bucket));
...
SH_LOOKUP_HASH_INTERNAL(SH_TYPE * tb, SH_KEY_TYPE key, uint32 hash)
{
	...
	{
		SH_ELEMENT_TYPE *entry;
		gamma_hash_bucket *bucket = &tb->buckets[curelem];
...
SH_START_ITERATE(SH_TYPE * tb, SH_ITERATOR * iter)
{
	...
	{
		gamma_hash_bucket *bucket = &tb->buckets[i];
...
SH_ITERATE(SH_TYPE * tb, SH_ITERATOR * iter)
{
	while (!iter->done)
	{
		gamma_hash_bucket *bucket = &tb->buckets[iter->cur];
